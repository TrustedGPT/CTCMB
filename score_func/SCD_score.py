...
def scd_score(data: dict, llm: LlmChat, model:str) -> dict:
    ...
    answer = {k: v for k, v in data.items() if k == "optimized_past_medical_history" or k=="optimized_present_illness"}
    answer_deal = json.dumps(answer, ensure_ascii=False, indent=4)
    log_message = (f"[{os.getpid()}] question: {data}\nresponse: {response}, \n")
